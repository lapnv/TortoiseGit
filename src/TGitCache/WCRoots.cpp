...
__int64 CWCRoots::GetDBFileTime(const CTGitPath& path)
{
    AutoLocker lock(m_critSec);
    auto it = m_WCDBs.lower_bound(path);
    if (it != m_WCDBs.end())
    {
...
std::map<CTGitPath, WCRootsTimes>::iterator CWCRoots::AddPathInternal(const CTGitPath& path)
{
    AutoLocker lock(m_critSec);
    CTGitPath p(path);
    do
...
bool CWCRoots::AddPath(const CTGitPath& path)
{
    return AddPathInternal(path) != m_WCDBs.end();
}

bool CWCRoots::NotifyChange(const CTGitPath& path)
{
    AutoLocker lock(m_critSec);
    CTGitPath p(path);
    bool changed = true;
