...
public:
    ...
    __int64 GetDBFileTime(const CTGitPath& path);
    ...
    bool NotifyChange(const CTGitPath& path);

private:
    std::map<CTGitPath, WCRootsTimes>::iterator AddPathInternal(const CTGitPath& path);

    CComAutoCriticalSection                 m_critSec;
    std::map<CTGitPath, WCRootsTimes>       m_WCDBs;
};
